...
public class flipDown extends Command {
  ...
  public void end(boolean interrupted) {
    flippy.leftPivot.setVoltage(feedforward.calculate(flippy.wristAngle, 0));
    flippy.rightPivot.setVoltage(feedforward.calculate(flippy.wristAngle, 0));
  ...
  public boolean isFinished() {
    if (Math.abs(pid.calculate(flippy.wristEncoder.get(), 225)) <.005){
      return true;
    }
    else{
      return false;
